...
const attemptedCapture = Math.abs(file) === 1 && rank === forward;
    ...
    if (Math.abs(rank) === 2) {
      ...
      const pieceBetweenCheck = board?.getPieceFromPosition(oneSpaceAhead);
      if (pieceBetweenCheck) return false;
    }
    if (board && board?.getPieceFromPosition(position) !== null) return false;
  ...
  availableMoves(board: ChessBoard): Position[] {
    ...
    if (leftFile >= 1) {
      ...
      if (this.canMoveTo(leftFilePosition, board)) moves.push(leftFilePosition);
    }
    if (rightFile < 8) {
      ...
      if (this.canMoveTo(rightFilePosition, board))
